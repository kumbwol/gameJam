...
export class Enemy2
{
    ...
    private onUpdate(): void
    {
        for (let i = 0; i < SpaceshipHandler._spaceships.length; i++) {
            if (SpaceshipHandler._spaceships[i]._tag == "enemy1") SpaceshipHandler._spaceships[i].setTargetPosition(this._targetPoint[0], this._targetPoint[1]);
        }
    }

    public static spawn(position: number[], id: number)
    {
        let spaceship = new Spaceship("spaceships/enemy1.png", "enemy1", 0x5555ff, id);
